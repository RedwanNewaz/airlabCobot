...
class MycobotTest(object):
    def __init__(self):
        ...
        self.win = tkinter.Tk()
        self.win.title("myArm test tool for Raspberry Pi")
        ...
        # Connect
        self.connect_label = tkinter.Label(self.win, text="Connect the machine：")
        ...
        self.check_btn = tkinter.Button(
            self.win, text="Start testing", command=self.check_mycobot_servos
        )
        self.check_btn.grid(row=4, column=1)

        # Calibration.
        self.calibration_num = None
        self.calibration_label = tkinter.Label(self.win, text="Calibrate the servo：")
    ...
    def connect_mycobot(self):
        ...
        if not port:
            self.write_log_to_Text("Please select serial port")
            return
    ...
    def disconnect_mycobot(self):
        ...
        try:
            ...
            self.write_log_to_Text("Not connected to the machine yet！！！")
    ...
    def check_mycobot_servos(self):
        ...
        if res:
            ...
            self.write_log_to_Text("All joints are connected properly。")
    ...
    def calibration_mycobot(self):
        ...
        time.sleep(0.1)
        self.write_log_to_Text("Calibrating motor %s ended." % self.calibration_num)
    ...
    def start_aging_test(self):
        if not self.has_mycobot():
            return
        self.write_log_to_Text("The cycle aging test needs to last for 30 minutes and will automatically stop after 30 minutes.")
        self.aging_stop = False
    ...
    def stop_aging_test(self):
        try:
            ...
            self.write_log_to_Text("End burn-in test failed ！！！")
    ...
    def rectify_mycobot(self):
        ...
        for i in range(1, 8):
            ...
            time.sleep(0.1)
        self.write_log_to_Text("Enable all motors to complete.")
    ...
    def has_mycobot(self):
        ...
        if not self.mycobot:
            self.write_log_to_Text("Not connected to myArm yet！！！")
            return False
        return True
